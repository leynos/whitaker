...
use thiserror::Error;
use validation::{lint_constant, normalize_crate_name, normalize_ui_directory, pass_struct_name};
...
impl LintCrateTemplate {
    ...
    pub fn with_ui_tests_directory(
        ...
        let name = normalize_crate_name(crate_name.as_ref())?;
        let lint_constant = lint_constant(&name);
        let pass_struct = pass_struct_name(&name);
        let ui_directory = normalize_ui_directory(ui_tests_directory.as_ref())?;
