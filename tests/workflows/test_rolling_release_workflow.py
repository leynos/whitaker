...
def test_install_components_uses_only_matrix_target_rustc_dev() -> None:
    ...
    rustc_dev_occurrences = sum("rustc-dev" in line for line in install_lines)
    llvm_tools_occurrences = sum(
        "llvm-tools-preview" in line for line in install_lines
    )
    assert (
        rustc_dev_occurrences == 1
    ), (
        "'rustc-dev' must appear exactly once in install script, found "
        f"{rustc_dev_occurrences}"
    )
    assert (
