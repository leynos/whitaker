...
/// Reports whether `name` has the required case-sensitive `.tgz` suffix.
fn has_tgz_suffix(name: &str) -> bool {
    name.ends_with(".tgz")
}
...
fn archive_filename_tgz_for_non_windows(#[case] target: &str) {
    ...
    assert!(has_tgz_suffix(&name), "expected .tgz suffix, got {name}");
...
fn package_installer_creates_archive(
    ...
    let entries = if has_tgz_suffix(expected_name) {
