...
/// Deprecated alias for [`rasterize_signal`].
///
/// # Errors
///
/// Propagates the same [`SignalBuildError`] conditions as [`rasterize_signal`]:
/// a zero (non-one-based) line in the range, an inverted range, or a segment
...
pub fn smooth_moving_average(signal: &[f64], window: usize) -> Result<Vec<f64>, SmoothingError> {
    ...
    for idx in 0..signal.len() {
        ...
        let sum = prefix.get(end + 1).copied().unwrap_or(0.0)
            - prefix.get(start).copied().unwrap_or(0.0);
