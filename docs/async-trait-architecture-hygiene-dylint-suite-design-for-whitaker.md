...
- ADR 006: “Dual-trait pattern for dyn-backed async interfaces” (accepted
  shape, BoxFuture alias convention, blanket adapter, and migration
  gotchas).[^1]
...
3. Prevent unnecessary use of `async-trait`-style boxing when the trait is not
   ...
   than expected once dyn usage and supertrait inheritance were accounted
   for.[^3]
...
those are treated as first-class lint rules rather than “docs-only” advice.[^1]
[^3]
...
`AsyncTraitFamilyIndex` should live in the `whitaker` crate (not `common`) under
`#[cfg(feature = "dylint-driver")]`, alongside existing HIR helpers.[^2] This
keeps the index available to lint crates without forcing compiler dependencies
into `common`, consistent with other Whitaker designs that keep pure logic
separate from extraction.[^2]
...
boilerplate at compile time.[^3] The `async-trait` crate documentation confirms
the core mechanical transformation into boxed futures and the default `Send`
behaviour, which are exactly the costs and semantics teams must make explicit
when migrating.[^5]
...
boxed-future alias (“one helper module”) to reduce signature verbosity.[^1][^3]
Whitaker lints should support both “one canonical alias” and “family-local
alias” modes, because some codebases prefer keeping interface types private to
the module. The trade-off is between global consistency (better for large
migrations) and local encapsulation.
