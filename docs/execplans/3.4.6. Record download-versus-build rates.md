...
installation duration so migration outcomes from Architecture Decision Record
(ADR-001) can be measured.

After this work, each successful non-dry-run installation will persist one
local metric event and update aggregated counters. The installer will surface a
short summary showing:

- download rate (fraction of installations that used prebuilt artefacts);
- build rate (fraction of installations that built locally, including fallback);
- total installation time accumulated across recorded installations.
...
- Record metrics only for successful installation completion to avoid skew from
...
- Iterations: if quality gates fail after 5 fix cycles, stop and escalate with
  options.
- Ambiguity: if "record rates" must include failed installations or partial
  runs, stop and confirm policy before proceeding.
...
- Risk: ambiguous interpretation of "download-versus-build rates".
  ...
  denominator as successful installations and record this decision in the
  design document.
...
- Risk: duration measurement becoming inconsistent across code paths.
  ...
  around installation orchestration and feed a single terminal outcome into
  metrics recorder.
...
- No existing installer metrics module or persisted installation-statistics file
...
- `make test` can take over 20 minutes in this environment due to long-running
  UI and behavioural scenarios, so log capture and patient polling are required.

## Decision log

- Decision: Persist aggregated installer metrics in a local JSON file under
  Whitaker's data directory, and update it after successful installations.
...
- Decision: Define rates over successful completed installations only.
...
- Added `installer/src/install_metrics.rs` with:
  ...
  - download/build counters and cumulative installation duration,
  - rate calculations and summary-line formatting,
  - corrupt-file recovery and non-fatal persistence error handling.
- Integrated metrics recording into successful installation completion paths in
...
- Quality gates passed:
  - `make check-fmt`,
  - `make lint`,
  - `make test` (634 passed, 2 skipped).

## Context and orientation

Current relevant flow:

- `installer/src/main.rs` orchestrates installation flow in `run_install()`,
  trying prebuilt first and falling back to local build.
...
Record total installation duration and terminal installation mode in one place.
...
- Capture `Instant::now()` at start of non-dry-run installation path.
...
- After successful `finish_install(...)`, update metrics store and emit a short
  summary line containing download rate, build rate, and cumulative
  installation time.
...
- Unit tests verify mode classification and that metrics update is skipped for
  dry-run and failed installations.
...
- Happy: first successful download installation creates metrics file and rates
  are `download=1.0`, `build=0.0`.
- Happy: subsequent build installation updates counts and cumulative duration.
- Unhappy: malformed metrics JSON resets to defaults and still records the
  current installation.
- Unhappy: metrics file write failure returns warning path without aborting the
  installation.
...
Required Behaviour-Driven Development (BDD) scenarios:

- Happy: successful prebuilt path records one download installation and updates
  total time.
- Happy: fallback/local-build path records one build installation and updates
  total time.
- Unhappy: unreadable/corrupt prior metrics state is recovered and recording
  still proceeds.
- Unhappy: metrics persistence failure reports warning and installation still
  completes.
...
- Installer records aggregate metrics for successful installations and surfaces
...
- Metrics updates are append-safe in the sense of repeated successful
  installations; each rerun increments counters deterministically.
...
- `make lint`: success.
- `make test`: `634 tests run: 634 passed, 2 skipped`.
...
status/progress to complete, and recorded outcomes plus validation evidence.
