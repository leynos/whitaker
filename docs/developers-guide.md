...
The target packages `whitaker-installer` into an isolated scratch target
directory, extracts the resulting `.crate`, then uses Rust 1.85 to install that
package with `--locked` into a scratch root and run `--version`. It clears the
scratch directory before it starts and removes it on exit. This checks the
...
33748602187 and 33756048103 showed. It also fails when write errors exceed
`ERROR_RATE_LIMIT`, one tenth, of the store attempts (writes plus write
errors), or lookup timeouts exceed it of the reads (hits plus misses).
`sccache` counts a timed-out lookup as a miss, so a timeout is judged against
the reads and never the stores: a warm lane stores almost nothing, and one
timeout beside one store would otherwise read as a 100% failure. Read errors
have no rate of their own. `sccache` declares `cache_read_errors` but never
increments it, and a failed read lands in `cache_errors` beside compile errors
that say nothing about the cache. Exceeding the limit fails; reaching it does
not. The measured healthy rate is about 0.1%: the cold run 35672193433 had one
write error and one timeout against 1,572 store attempts in `coverage-check`
and 1,212 in `linux-full`. So the limit sits two orders of magnitude above
normal noise and still catches an endpoint that loses one compile in ten.
Isolated errors below it produce warnings instead. A proxy hiccup costs one
...
not readable off a `runs-on`. Each declares `${{ matrix.os }}` and GitHub runs
it five times, on five images; in both, the two Linux legs are on Ubicloud and
the other three are not. The contracts therefore speak of a *lane*, which is a
job together with the matrix leg that selected its runner, written
`build-lints[target]`. A job with no matrix is a lane whose leg is absent.
`tests/workflow_contracts/runner_lanes.py` resolves a lane to the label it
actually runs on, by reading the matrix entry rather than by evaluating the
expression: the only expression it understands is the deferral to the matrix,
and any other is refused rather than guessed at, because reporting an invented
label would be worse than reporting that it cannot be read.

Two consequences follow, and both are held by contract.

The x86_64 legs' image is not a default. Both are on
`ubicloud-standard-2-ubuntu-2204` while every other Ubicloud lane here is on
2404, because the glibc those legs link against is the floor every consumer of
whitaker's binaries inherits, and each job's own "Check glibc baseline" step
holds it at `GLIBC_2.35`, which is what 22.04 ships. The two jobs publish
artefacts a consumer installs side by side, so the floor is the higher of the
two and neither may be raised alone. In each job the image and the ceiling are
pinned together, since either alone is defeatable: an image can be raised while
the check still names the old ceiling, and the ceiling can be raised while the
image is untouched. The arm64 legs stay on the 2404 arm image they have always
built on, so this placement changed where they run and not what they link
...
ordinary pushes and unforced manual runs retain the rolling release's
documented partial-publication policy. `scripts/check-forced-rolling-assets.sh`
owns this local preflight for the rolling workflow's publish job. It is not a
tagged-release or consumer asset validator; those paths keep their own checks.

Each sidecar has one GNU checksum record: a digest, a text marker of two spaces
or a binary marker of a space followed by `*`, and the exact path
`dist/<archive>`. The gate hashes the expected archive and does not follow a
different path supplied by a sidecar.
