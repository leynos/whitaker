...
mod fixture_tests {
    ...
    #[test]
    fn resolve_fixture_config_reads_workspace_fixture() {
        let directory = Utf8PathBuf::from("ui");
        let source = directory.join("pass_custom_limit.rs");
        let config = resolve_fixture_config(directory.as_ref(), source.as_std_path()).unwrap();

        let contents = config.expect("fixture config should be present");
        assert!(contents.contains("max_branches = 3"));
    }
}
