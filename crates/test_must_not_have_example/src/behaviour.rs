...
impl DocumentationWorld {
    fn push_line(&self, line: &str) { self.lines.borrow_mut().push(line.to_owned()); }
...
fn given_line(world: &DocumentationWorld, line: String) { world.push_line(line.trim_matches('"')); }

#[when("I evaluate the documentation")]
fn when_evaluate(world: &DocumentationWorld) { world.evaluate(); }
...
fn scenario_examples_heading(world: DocumentationWorld) { let _ = world; }

#[scenario(path = "tests/features/doc_examples.feature", index = 1)]
fn scenario_code_fence(world: DocumentationWorld) { let _ = world; }

#[scenario(path = "tests/features/doc_examples.feature", index = 2)]
fn scenario_inline_ticks(world: DocumentationWorld) { let _ = world; }

#[scenario(path = "tests/features/doc_examples.feature", index = 3)]
fn scenario_plain_prose(world: DocumentationWorld) { let _ = world; }

#[scenario(path = "tests/features/doc_examples.feature", index = 4)]
fn scenario_source_order(world: DocumentationWorld) { let _ = world; }
